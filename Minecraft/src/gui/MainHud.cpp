...
namespace Minecraft
{
	namespace MainHud
	{
		...
		static const int numInventoryRows = 3;
		static const int numInventoryColumns = 9;
		...
		static std::array<glm::vec2, numHotBarInventorySlots + numInventoryRows * numInventoryColumns> slotPositions;
		...
		static void initSlotPositions()
		{
			...
			for (int row = 0; row < numInventoryRows + 1; row++)
			{
				for (int column = 0; column < numInventoryColumns; column++)
				{
					slotPositions[column + (row * numInventoryColumns)] = currentSlotPosition;
